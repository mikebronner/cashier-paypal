...
6. Prepare the billable model (typically the default Laravel User model):
    ...
    - Add the `GeneaLabs\CashierPaypal\Billable` trait.
    ...
    - Implement `GeneaLabs\CashierPaypal\Order\Contracts\ProvidesInvoiceInformation` interface. For example:
...
use GeneaLabs\CashierPaypal\SubscriptionBuilder\RedirectToCheckoutResponse;
...
Additionally, listen for the following events (in the `GeneaLabs\CashierPaypal\Events` namespace) to add app specific behaviour:
...
Listen for the `OrderInvoiceAvailable` event (in the `GeneaLabs\CashierPaypal\Events` namespace).
...
You can listen for the following events from the GeneaLabs\CashierPaypal\Events namespace:
